...
(defn navbar
  ...
  (let [{:keys [new error ok]} (tree/counts tree)]
    [:nav {:class "bg-white text-gray-500 border-gray-200 divide-gray-200
  px-2 sm:px-4 py-2.5 fixed w-full z-20 top-0 start-0 border-b"}
     [:div.mx-auto.flex.flex-wrap.justify-between.items-center.container
      ...
      [:div.mx-0.inline-flex
       ...
       [:div.flex.md:order-2.space-x-2
        [nav-button [:<> [icon-play] "Replay All"]]
        [nav-button [:<> [icon-floppy-disk] "Save"]]
        [nav-button "Undo"]
        [nav-button "Redo"]
        [nav-button "Quit"]]]]]))

(defn render-knot
  [{:keys [id response] :as knot}]
  [:div.border-x-4.border-slate-100 {:id (str "knot-" id)}  ;; TODO: Color
