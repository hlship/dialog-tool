...
(deftest collect-replay-to-test
  (testing "apply-responses sets :unblessed when response differs"
    (let [tree     (-> (make-tree)
                       (tree/add-child 0 1 "look" (response "Old response.")))
          session  (make-session tree)
          result   (session/apply-responses session {1 [(response "New response.") nil]})
  ...
  (testing "apply-responses clears :unblessed when response matches blessed :response"
    ...
    (let [tree     (-> (make-tree)
                       (tree/add-child 0 1 "look" (response "Same response."))
                       (tree/bless-response 1)
                       (tree/update-response 1 (response "Different.")))
          session  (make-session tree)
          result   (session/apply-responses session {1 [(response "Same response.") nil]})
          new-knot (get-in result [:tree :knots 1])]
